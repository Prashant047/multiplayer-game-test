...
var gameBoard = document.getElementById('can');
...
var ctx = gameBoard.getContext('2d'); // game rendering context;

exports.ctx = ctx;
exports.gameBoardDimentions = gameBoardDimentions;
...
var socket = (0, _socket2.default)('http://192.168.1.16:3001');
...
var _gameSettings = __webpack_require__(6);

var _vehicle = __webpack_require__(24);

var _socket = __webpack_require__(10);

var vehicles = [];
var myVehicle = void 0;
// Handeling socket stuff ------------
_socket.socket.on('connect', function () {
    console.log('connected to the server');
});

_socket.socket.on('init', function (data) {

    if (vehicles.length == 0) {
        var newVehicle = new _vehicle.Vehicle(data.newVehicle.position, data.newVehicle.color, data.newVehicle.angle, data.newVehicle.id);
        console.log("initialized myself");
        myVehicle = newVehicle;
        vehicles.push(myVehicle);
        myVehicle.activateListener();
        data.oldVehicles.forEach(function (v, index) {
            vehicles.push(new _vehicle.Vehicle(v.position, v.color, v.angle, v.id));
        });
    } else {
        vehicles.push(new _vehicle.Vehicle(data.newVehicle.position, data.newVehicle.color, data.newVehicle.angle, data.newVehicle.id));
...
_socket.socket.on('turn-left', function (data) {
...
_socket.socket.on('turn-right', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
            vehicle.isRotating = true;
...
_socket.socket.on('move-backwards', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
...
_socket.socket.on('move-forwards', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
            vehicle.isMoving = true;
...
_socket.socket.on('stop-turning-right', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
...
_socket.socket.on('stop-turning-left', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
            vehicle.isRotating = false;
            vehicle.turningLeft = false;
...
_socket.socket.on('stop-moving-forwards', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
...
_socket.socket.on('stop-moving-backwards', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
            vehicle.isMoving = false;
            vehicle.movingForwards = false;
...
_socket.socket.on('fire-bullets', function (data) {
    vehicles.forEach(function (vehicle, index) {
        if (vehicle.id == data.id) {
...
_socket.socket.on('bye', function (data) {
...
var gameLoop = function gameLoop() {
    _gameSettings.ctx.clearRect(0, 0, _gameSettings.gameBoardDimentions.width, _gameSettings.gameBoardDimentions.height);
...
var _cannon = __webpack_require__(25);
...
var Vehicle = function () {
    function Vehicle(position, color, angle, id) {
        ...
        this.cannon = new _cannon.Cannon(this.x + this.noseLength * Math.cos(this.angle), this.y + this.noseLength * Math.sin(this.angle), this.angle, this.color);
    }

    _createClass(Vehicle, [{
        key: 'activateListener',
        value: function activateListener() {
            var _this = this;
            ...
            window.addEventListener('keypress', function (e) {
                ...
                if (e.keyCode == 32) {
                    ...
                    _socket.socket.emit('firing-bullets', {
                        id: _this.id
                    });
                }
            });
            ...
            window.addEventListener('keydown', function (e) {
                ...
                if (e.keyCode == 39) {
                    ...
                    _socket.socket.emit('turning-right', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
                ...
                if (e.keyCode == 37) {
                    ...
                    _socket.socket.emit('turning-left', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
                ...
                if (e.keyCode == 38) {
                    ...
                    _socket.socket.emit('moving-forwards', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
                ...
                if (e.keyCode == 40) {
                    ...
                    _socket.socket.emit('moving-backwards', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
            ...
            window.addEventListener('keyup', function (e) {
                ...
                if (e.keyCode == 39) {
                    ...
                    // this.turningLeft = false;
                    // this.turningRight = false;
                    _socket.socket.emit('stopped-turning-right', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
                ...
                if (e.keyCode == 37) {
                    ...
                    // this.turningLeft = false;
                    // this.turningRight = false;
                    _socket.socket.emit('stopped-turning-left', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
                ...
                if (e.keyCode == 38) {
                    ...
                    // this.movingForwards = false;
                    // this.movingBackwards = false;
                    _socket.socket.emit('stopped-moving-forwards', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
                ...
                if (e.keyCode == 40) {
                    ...
                    // this.movingForwards = false;
                    // this.movingBackwards = false;
                    _socket.socket.emit('stopped-moving-backwards', {
                        id: _this.id,
                        position: {
                            x: _this.x,
                            y: _this.y
        ...
        key: 'updateCannonAngle',
        value: function updateCannonAngle() {
        ...
        key: 'updateCannonPosition',
        value: function updateCannonPosition() {
        ...
        key: 'draw',
        value: function draw() {
            _gameSettings.ctx.beginPath();
            _gameSettings.ctx.moveTo(this.x + this.noseLength * Math.cos(this.angle), this.y + this.noseLength * Math.sin(this.angle));

            _gameSettings.ctx.lineTo(this.x + this.noseLength * Math.cos(2 * Math.PI / 3 + this.angle) / 3, this.y + this.noseLength * Math.sin(2 * Math.PI / 3 + this.angle) / 3);

            _gameSettings.ctx.lineTo(this.x, this.y);

            _gameSettings.ctx.lineTo(this.x + this.noseLength * Math.cos(4 * Math.PI / 3 + this.angle) / 3, this.y + this.noseLength * Math.sin(4 * Math.PI / 3 + this.angle) / 3);

            _gameSettings.ctx.closePath();
            _gameSettings.ctx.fillStyle = this.color;
            _gameSettings.ctx.fill();
        }
    }]);
...
var Bullet = function () {
    ...
    _createClass(Bullet, [{
        ...
        value: function draw() {
            _gameSettings.ctx.beginPath();
            _gameSettings.ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2);
            _gameSettings.ctx.closePath();
            _gameSettings.ctx.fillStyle = this.color;
            _gameSettings.ctx.fill();
        }
...
var Cannon = function () {
    function Cannon(x, y, angle, color) {
    ...
    _createClass(Cannon, [{
        ...
        value: function fireBullet() {
            var bullet = new Bullet(this.x, this.y, this.bulletVelocity, this.color);
        ...
        value: function update() {
            ...
            for (var i = 0; i < this.bullets.length; i++) {
                if (this.bullets[i].x - this.bullets[i].radius > _gameSettings.gameBoardDimentions.width || this.bullets[i].x + this.bullets[i].radius < 0) {
                    this.bullets.splice(i, 1);
                    console.log('removed x');
                    continue;
                }
                if (this.bullets[i].y - this.bullets[i].radius > _gameSettings.gameBoardDimentions.height || this.bullets[i].y + this.bullets[i].radius < 0) {
                    this.bullets.splice(i, 1);
